/**
 * Noesis Preview — Mintlify-styled documentation viewer for Pneuma.
 *
 * Renders manifest.json + Markdoc content with theme toggle,
 * ToC sidebar, and structured content components.
 */
...
export default function Preview({ files, selection, onActiveFileChange }: ViewerPreviewProps) {
  ...
  const manifest = useMemo<SiteManifest | null>(() => {
    const f = files.find((f) => f.path === "manifest.json");
    if (!f) return null;
    try { return JSON.parse(f.content); } catch { return null; }
  }, [files]);

  const allPages = useMemo(() => manifest?.sections.flatMap((s) => s.pages) ?? [], [manifest]);

  const [localActiveFile, setLocalActiveFile] = useState("");
  useEffect(() => { if (selection?.file) setLocalActiveFile(selection.file); }, [selection?.file]);
  const activeFile = localActiveFile || allPages[0]?.file || "";
